...
class Layers( QObject ):
    ...
    def __init__( self, iface, parent=None ):
        ...
        self._qmldir = os.path.join(os.path.dirname(os.path.abspath(__file__)),'styles')
        self._formdir = os.path.join(os.path.dirname(os.path.abspath(__file__)),'forms')
    ...
    def endFeatureEdits( self, cancel=False ):
        ...
        if modified and self._name and not cancel:
            ...
            result = QMessageBox.question( self._iface.mainWindow(),
    ...
    def zoomToFeature( self ):
        extentWkt = self._database.scalar('select ST_AsText(gazetteer.gaz_featureExtents(:feat_id,:margin))',
                                    feat_id=self._featid, margin=Layers.zoomMargin)
        if not extentWkt:
            return
        if not self._transform:
            return
        ...
        mapview = self._transform.transformBoundingBox( geom.boundingBox(), QgsCoordinateTransform.ReverseTransform )
    ...
    def addSelectedGeometries( self ):
        ...
        gtypes = ((QGis.Point,'point','fpoint'),(QGis.Line,'line','fline'),(QGis.Polygon,'polygon','fpoly'))
        summary = []
        for gtype in gtypes:
        ...
        result = QMessageBox.question(self._iface.mainWindow(),
                "Add geometries",message,QMessageBox.Ok | QMessageBox.Cancel)
    ...
    def deleteSelectedGeometries( self ):
        ...
        result = QMessageBox.question(self._iface.mainWindow(),
                "Delete geometries",message,QMessageBox.Ok | QMessageBox.Cancel)
        if result != QMessageBox.Ok:
            return
