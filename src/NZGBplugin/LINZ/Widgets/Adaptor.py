...
class Adaptor( object ):
    ...
    class AttDef:
        ...
        def isobject(self): return self._isobject
        def type(self): return self._type
        def editable(self): return self._editable
        def islist(self): return self._islist
    ...
    def addAttribute( self, attribute, atype, editable=False, isid=False, islist=False ):
        ...
        if not isinstance(atype,type) and not isinstance(atype,Adaptor):
            raise RuntimeError(str(atype)+' is not an type in ' + self.__class__.__name__)
    ...
    def setIdAttribute( self, idattribute ):
        ...
        if idattribute not in self._attrdef:
            raise RuntimeError('Invalid id '+idattribute+' specified for '+self.typename()+' adaptor')
    ...
    def typename( self, object=None ):
        ...
        '''
        return (self._typename if self._typename else 
                object.__class__.__name__ if object != None else
    ...
    def _getObjectValue( self, object, attribute ):
        ...
        '''
        raise RuntimeError( self.__class__.__name__ + ' needs to override _setObjectValue')
    ...
    def _getMemberObject( self, object, attribute ):
        ...
        if not member in self._attrdef:
            raise RuntimeError( member + ' is not defined in the Adaptor for ' + self.typename(object))
        attrdef = self._attrdef[member]
        if attrdef.islist():
            raise RuntimeError( 'Cannot retrieve ' + memberattr + ' of list member ' + member + ' of ' + self.typename(object))
        if not attrdef.isobject():
            raise RuntimeError( attribute + ' is not defined in the Adaptor for ' + self.typename(object))
    ...
    def getValue( self, object, attribute ):
        ...
        if attribute not in self._attrdef:
           raise RuntimeError("Invalid attribute " + attribute + " of "+self.typename(object))
    ...
    def setValue( self, object, attribute, value, overwrite=False ):
        ...
        if '.' in attribute:
            ...
            if not self._attrdef[member].editable():
                raise RuntimeError("Cannot edit the "+attribute+" attribute of "+self.typename(object))
            adaptor, member, attribute = self._getMemberObject( object, attribute )
        ...
        attrdef = self._attrdef[attribute]
        if attrdef.islist():
            raise RuntimeError("Attribute "+attribute+" of "+self.typename(object)+" cannot be updated as it is a list attribute")
        if attrdef.isobject():
            raise RuntimeError("Attribute "+attribute+" of "+self.typename(object)+" cannot be used without member attribute qualifier")
        if not attrdef.editable() and not overwrite:
            raise RuntimeError("The "+attribute+" attribute of "+self.typename(object)+" cannot be edited")
        if value != None:
            t = attrdef.type()
            try:
                value = t(value)
            except:
                raise RuntimeError("Cannot convert " + str(value) + " to " + t.__name__ + " for " + self.typename(object)+'.'+attribute)
