...
from qgis.PyQt.QtCore import pyqtSignal, QModelIndex, QAbstractTableModel, Qt
...
class DictionaryListView(QTableView):
    ...
    def setModel(self, model):
        ...
        self._model = model
        self._dictionaryList = (
            self._model if isinstance(self._model, DictionaryListModel) else None
        )
        if self._model:
    ...
    def _restoreSelectedRow(self):
        if not self.selectId(self._selectedId) and not self.selectId(
            self._alternativeId
        ):
            self.selectRow(0)
...
class DictionaryListModel(QAbstractTableModel):
    ...
    def _createIndex(self):
        if self._filter:
            self._index = [
                i for i in range(len(self._list)) if self._filter(self._list[i])
            ]
        else:
    ...
    def data(self, index, role):
        ...
        if role == Qt.DisplayRole or role == Qt.EditRole:
            ...
            and not self._editable[col]
            and self._readonlyBrush
        ):
    ...
    def updateItem(self, index):
        ...
        if self._filter:
            ...
            self.dataChanged.emit(
                self.index(row, 0), self.index(row, len(self._columns))
            )
