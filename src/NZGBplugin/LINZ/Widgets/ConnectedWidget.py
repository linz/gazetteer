...
class ConnectedWidget( QWidget ):
    ...
    def load( self, object_, isNew=False, overwrite=False):
        ...
        if not self._connector:
            raise RuntimeError("Cannot use load before setAdaptor in "+self.objectName())
    ...
    def querySave( self ):
        ...
        if self._object == None or not self._connector.isDirty():
            return True
        result = QMessageBox.question(self,"Save "+self._typename+"?","Do you want to save the "+self._typename+"?",
    ...
    def validate( self ):
        ...
        if not isok:
            QMessageBox.warning(self,"Errors in "+self._typename,"\n".join(messages))
            if widget:
