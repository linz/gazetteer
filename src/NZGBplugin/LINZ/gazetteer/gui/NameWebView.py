...
class NameWebView(QWebView):
    ...
    class Template(QObject):
            ...
            if self._template == None or self._stat != os.path.getmtime(self._file):
                try:
                    if not os.path.exists(self._file):
                        raise ValueError(
                            "Template file " + self._file + " does not exist"
                        )
                    ...
                    def tfun(**kwords):
                        return (
                            "<html><head><title>Error</title></head><body><h1>Error</h1><p>"
        ...
        def lookupOptions(self, code_group, showDescriptions=False):
            ...
            vfunc = lambda x: x.value + (
                " (" + x.description + ")" if showDescriptions and x.description else ""
            )
        ...
        def sortAnnotations(self, code, annotations):
            ...
            return sorted(
                annotations,
                key=lambda x: lookup.get(x.annotation_type, "9999") + x.annotation_type,
            )
            ...
            for item in editdata["new"]:
                ...
                if objtype == "Association":
                    ...
                    try:
                        ...
                        raise ValueError(
                            "Invalid object type " + str(objtype) + " for new object"
                        )
    ...
    def reload(self):
        ...
        if self.Debug:
            ...
            with codecs.open(
                os.path.join(self._basedir, "html", "dump.html"), "w", "utf8"
            ) as f:
                f.write(html)
