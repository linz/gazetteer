...
class User(base):
    __table__ = Table('gazetteer_users', meta, PrimaryKeyConstraint('userid'), autoload=True )
...
class SystemCode(base):
    ...
    def codeGroup( code_group, refresh=False ):
        if refresh or code_group not in SystemCode.__codegroups__:
            group=list(Database.query(SystemCode).filter(SystemCode.code_group==code_group).all())
...
class Feature(base):
    ...
    annotations = relationship('FeatureAnnotation',backref='feature',cascade="all, delete-orphan")
