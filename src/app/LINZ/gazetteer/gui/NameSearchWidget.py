...
class NameSearchWidget( QWidget, Ui_NameSearchWidget):
    ...
    def _populateFeatTypeDropdown( self, index ):
        ...
        if fclass:
            FormUtils.populateCodeCombo(self.uSearchFeatType,'FTYP',True, category=str(fclass))
        else:
    ...
    def _populateRecent( self ):
        ...
        self._populateResultList( self.uRecentNames, self._controller.recent(allusers,editonly,nmax))
    ...
    def eventFilter( self, widget, event):
        if event.type() == QEvent.KeyPress:
            if widget == self.uSearchText and event.key() in [Qt.Key_Down, Qt.Key_Enter, Qt.Key_Return]:
                self._doSearch()
    ...
    def _selectNameId( self, name_id ):
        self.nameSelected.emit( name_id, (QApplication.keyboardModifiers() & Qt.ShiftModifier) == Qt.ShiftModifier )
