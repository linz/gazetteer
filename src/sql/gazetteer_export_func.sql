...
-- FUNCTION: gazetteer_export.gaz_update_export_database()
-- DROP FUNCTION gazetteer_export.gaz_update_export_database();
CREATE OR REPLACE FUNCTION gazetteer_export.gaz_update_export_database(
	)
    RETURNS integer
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	...
	--Tangata Whenua names
    CREATE TABLE gazetteer_export.tangata_whenua_names AS
    SELECT
        NEX.name_id,
        NEX.name,
        NEX.status,
        ...
        NEX.ref_point
    FROM
        gazetteer.name_export NEX
	JOIN gazetteer.tangata_whenua_name TWN ON TWN.name_id = NEX.name_id;
	
    ALTER TABLE gazetteer_export.tangata_whenua_names ADD PRIMARY KEY (name_id);
    ALTER TABLE gazetteer_export.tangata_whenua_names  OWNER TO gazetteer_dba;
    GRANT SELECT ON gazetteer_export.tangata_whenua_names TO gazetteer_export;
    GRANT SELECT ON gazetteer_export.tangata_whenua_names  TO gaz_web_reader;
    GRANT ALL ON gazetteer_export.tangata_whenua_names TO gazetteer_dba;
        
    RETURN 1;
END
$BODY$;
ALTER FUNCTION gazetteer_export.gaz_update_export_database()
    OWNER TO gazetteer_dba;
GRANT EXECUTE ON FUNCTION gazetteer_export.gaz_update_export_database() TO gazetteer_dba;
REVOKE ALL ON FUNCTION gazetteer_export.gaz_update_export_database() FROM PUBLIC;